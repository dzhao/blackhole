...
use blackhole_client::client::FeatureClient;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = FeatureClient::new("http://192.168.86.247:8081").await?;
    loop {
        let (ids, features) = (
        ...
        let mut results = vec![];
        client.fetch_features_into(ids, &features, |_, _, values| results.extend_from_slice(values)).await?;
